...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Dans son modèle product_details est décrite comme étant de type Array. Or on stocke à l'intérieur
      //  un tableau d'objet. Lorsque l'on modifie un élément qui n'est pas explicitement prévu dans le modèle,
      //   le .save() ne suffit pas à sauvegardr les mofications. On doit le notifier de la sorte avant la
      //   sauvegarde afin qu'elle soit bien prise en compte. (Voir pour aller plus loin => Schemas, Models
      //      & markModified)
...
router.delete("/offer/delete/:id", isAuthentificated, async (req, res) => {
  try {
    ...
    // Je vais chercher l'offre dans mongoDB et je la supprime
